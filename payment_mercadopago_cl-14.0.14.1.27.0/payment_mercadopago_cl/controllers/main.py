...
#       id = identificador-de-la-operación
        ...
        if (tx):
            ...
            res = request.env['payment.transaction'].sudo().form_feedback( post, 'mercadopago_cl')

        return res

    @http.route('/payment/mercadopago_cl/ipn/', type='json', auth='none')
    ...
    @http.route('/payment/mercadopago_cl/dpn/', type='http', auth="none")
    ...
    @http.route('/payment/mercadopago_cl/cancel', type='http', auth="none")
